...
// Seed initial database files if they don't exist in the Persistent Disk
  ...
  files.forEach(file => {
    if (file.endsWith('.xlsx')) {
      ...
      if (!fs.existsSync(destPath)) {
        fs.copyFileSync(path.join(SEED_DIR, file), destPath);
        console.log(`Seeded ${file} to database.`);
      }
    }
  });
}
